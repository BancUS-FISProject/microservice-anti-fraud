...
describe('AntiFraudController (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    accountModel = moduleFixture.get<Model<AccountView>>(getModelToken(AccountView.name));
    alertModel = moduleFixture.get<Model<FraudAlert>>(getModelToken(FraudAlert.name));
    await app.init();
  });
  ...
  it('/v1/antifraud/transaction-check (POST) - Safe Transaction', async () => {
    ...
    return request(app.getHttpServer())
      ...
      .expect(200)
      .expect((res) => {
        expect(res.body.message).toBe('Transaction approved');
      });
  });
  ...
  it('/v1/antifraud/transaction-check (POST) - Fraud Detected', async () => {
    ...
    return request(app.getHttpServer())
      ...
      .expect(200)
      .expect((res) => {
        expect(res.body.message).toContain('Fraudulent behaviour detected');
      });
  });
  ...
  it('/v1/antifraud/accounts/:iban/fraud-alerts (GET) - Retrieve Alerts', async () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body[0].origin).toBe(VALID_IBAN_ORIGIN);
  ...
  it('/v1/antifraud/fraud-alerts/:id (PUT) - Update Alert Status', async () => {
    ...
    return request(app.getHttpServer())
      ...
      .expect((res) => {
        ...
        expect(res.body._id).toBe(alertId);
      });
  });
